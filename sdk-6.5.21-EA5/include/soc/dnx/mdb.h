...
#define DNX_KAPS_TRY(A)                                                                                              \
    ...
    {                                                                                                               \
        ...
        {                                                                                                           \
            SHR_IF_ERR_EXIT_WITH_LOG(mdb_lpm_kaps_error_translation(__tmp_status),                                       \
                                       "\n"#A" failed: %s%s%s\n",                                                   \
...
int mdb_lpm_physical_table_is_bb(
    int unit,
    dbal_enum_value_field_mdb_physical_table_e mdb_physical_table_id);
...
shr_error_e mdb_lpm_kaps_error_translation(
    kaps_status kaps_err);


kaps_status mdb_lpm_search(
    void *xpt,
    uint8_t * key,
...
kaps_status mdb_lpm_register_read(
    void *xpt,
    uint32_t offset,
    uint32_t nbytes,
    uint8_t * bytes);
...
kaps_status mdb_lpm_register_write(
    void *xpt,
    uint32_t offset,
    uint32_t nbytes,
    uint8_t * bytes);
