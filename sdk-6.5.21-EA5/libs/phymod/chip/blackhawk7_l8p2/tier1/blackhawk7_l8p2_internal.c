...
err_code_t blackhawk7_l8p2_INTERNAL_get_BER_string(srds_access_t *sa__, uint16_t time_ms, char *string, uint8_t string_size) {
    char string2[4];
    char string_extra[3] = "";
    struct blackhawk7_l8p2_ber_data_st ber_data_local = BER_DATA_ST_INIT;
...
err_code_t blackhawk7_l8p2_INTERNAL_eye_scan_setup(srds_access_t *sa__, uint8_t intr_pass_b, int16_t *y_max, int16_t *y_step) {
    INIT_SRDS_ERR_CODE
...
err_code_t blackhawk7_l8p2_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {
  ...
  EFUN(blackhawk7_l8p2_INTERNAL_display_BER(sa__,100));
  {
    ...
    if(state.ucv_status != 0) {
      EFUN_PRINTF(("\n"));
      EFUN(blackhawk7_l8p2_decode_uc_sts(sa__, state.ucv_status, decode_buf, sizeof(decode_buf), ""));
      EFUN_PRINTF(("%s", decode_buf));
    }
    if(state.ucv_status_ext != 0) {
      EFUN(blackhawk7_l8p2_decode_uc_sts_ext(sa__, state.ucv_status_ext, decode_buf, sizeof(decode_buf), ""));
      EFUN_PRINTF(("%s", decode_buf));
    }
  }
