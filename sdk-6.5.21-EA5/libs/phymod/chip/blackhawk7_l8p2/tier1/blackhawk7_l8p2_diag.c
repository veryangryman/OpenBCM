...
static err_code_t _blackhawk7_l8p2_display_eye_scan_ignored_lane(srds_access_t *sa__) {
    EFUN_PRINTF(("         : -|----|----|----|----|----|----|----|----|----|----|----|----|-"));
    return (ERR_CODE_NONE);
}
...
err_code_t blackhawk7_l8p2_display_eye_scan(srds_access_t *sa__) {
    ...
    eye_scan_err = blackhawk7_l8p2_INTERNAL_eye_scan_setup(sa__, 0, &y_max, &y_step);
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step)) {
        ...
        if(enable_read){
            eye_scan_err = blackhawk7_l8p2_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (eye_scan_err) {
                EFUN_PRINTF(("ERROR: blackhawk7_l8p2_read_eye_scan_stripe() returned: %s, (status = 0x%x)\n", blackhawk7_l8p2_INTERNAL_e2s_err_code(eye_scan_err), status));
                break;
            }
        }
...
err_code_t blackhawk7_l8p2_display_all_eye_scan(srds_access_t *sa__) {
    ...
    for(lane=0;lane < num_lanes;lane++) {
        ...
        err_code = blackhawk7_l8p2_INTERNAL_eye_scan_setup(sa__, 0, &i_max[lane], &i_step[lane]);
        if(err_code) {
            uint8_t core;
    ...
    EFUN(blackhawk7_l8p2_display_eye_scan_header(sa__, num_lanes));
        ...
        for(lane=0; lane < num_lanes; lane++) {
            if(lane_mask & (1<<lane)) {
                if(i[lane] >= -i_max[lane]) {
                    ...
                    if(enable_read) {
                        EFUN(blackhawk7_l8p2_read_eye_scan_stripe(sa__, &stripe[0], &status));
                    }

                    EFUN(blackhawk7_l8p2_display_eye_scan_stripe(sa__, i[lane],&stripe[buffer_idx]));
                    ...
                    if(eye_slice[lane]) {
                        ...
                        EFUN(_blackhawk7_l8p2_display_eye_scan_ignored_lane(sa__));
                    }
                }
            }
            else {
    ...
    EFUN(blackhawk7_l8p2_display_eye_scan_footer(sa__, num_lanes));
    for(lane=0;lane < num_lanes;lane++) {
        if(lane_mask & (1<<lane)) {
            EFUN(blackhawk7_l8p2_acc_set_lane(sa__, lane));
            EFUN(blackhawk7_l8p2_meas_eye_scan_done(sa__));
...
err_code_t blackhawk7_l8p2_display_eye_scan_header(srds_access_t *sa__, uint8_t i) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
...
err_code_t blackhawk7_l8p2_display_eye_scan_footer(srds_access_t *sa__, uint8_t i) {
    INIT_SRDS_ERR_CODE
    uint8_t x;
    uint8_t stepsize_var[NUM_LANES_MAX];
    uint8_t lane, num_lanes;
