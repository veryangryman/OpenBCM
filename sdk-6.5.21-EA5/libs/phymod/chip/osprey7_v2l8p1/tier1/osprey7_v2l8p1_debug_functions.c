...
err_code_t osprey7_v2l8p1_uc_lane_static_var_dump(srds_access_t *sa__, uint8_t core, uint8_t lane, char ram_buffer[][SRDS_DUMP_BUF_SIZE]) {
  INIT_SRDS_ERR_CODE
  uint8_t     rx_lock, uc_stopped = 0;
  uint16_t    addr, lane_static_var_ram_size;
  srds_info_t const * const osprey7_v2l8p1_info_ptr = osprey7_v2l8p1_INTERNAL_get_osprey7_v2l8p1_info_ptr_with_check(sa__);
  EFUN(osprey7_v2l8p1_INTERNAL_match_ucode_from_info(sa__, osprey7_v2l8p1_info_ptr));
  lane_static_var_ram_size = (uint16_t)osprey7_v2l8p1_info_ptr->lane_static_var_ram_size;
  ...
  if (ram_buffer == NULL) {
      ...
      for (addr = 0x0; addr < lane_static_var_ram_size; addr++) {
         if (!(addr % 26))  {
            EFUN_PRINTF(("\n%04x ",addr));
         }
         ESTM_PRINTF(("%02x ", osprey7_v2l8p1_rdblp_uc_var(sa__, __ERR, addr)));
      }
  }
  else {
      uint8_t rddata, index = 0;
      ...
      for (addr = 0x0; addr < lane_static_var_ram_size; addr++) {
         ...
         ESTM(rddata = osprey7_v2l8p1_rdblp_uc_var(sa__, __ERR, addr));
         count += USR_SNPRINTF(ram_buffer[index] + count,  (size_t)(SRDS_DUMP_BUF_SIZE - count - 1), "%02x ", rddata);
      }
      ram_buffer[++index][0] = 0xA;
      ram_buffer[++index][0] = 0;
