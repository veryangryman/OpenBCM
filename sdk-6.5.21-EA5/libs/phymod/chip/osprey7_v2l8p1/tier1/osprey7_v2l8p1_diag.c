...
err_code_t osprey7_v2l8p1_display_eye_scan(srds_access_t *sa__) {
    ...
    eye_scan_err = osprey7_v2l8p1_INTERNAL_eye_scan_setup(sa__, 0, &y_max, &y_step);
    ...
    for (y = y_max;y>=-y_max;y=(int16_t)(y-y_step)) {
        ...
        if(enable_read){
            eye_scan_err = osprey7_v2l8p1_read_eye_scan_stripe(sa__, &stripe[0], &status);
            if (eye_scan_err) {
                EFUN_PRINTF(("ERROR: osprey7_v2l8p1_read_eye_scan_stripe() returned: %s, (status = 0x%x)\n", osprey7_v2l8p1_INTERNAL_e2s_err_code(eye_scan_err), status));
                break;
            }
        }
...
Exit:
    ...
    if((diag_level & SRDS_DIAG_REG_CORE)
       || (diag_level & SRDS_DIAG_REG_LANE)) {
        diag_data_err = osprey7_v2l8p1_reg_dump(sa__, core, lane, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
    ...
    if(diag_level & SRDS_DIAG_UC_CORE) {
        diag_data_err = osprey7_v2l8p1_uc_core_var_dump(sa__, core, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
        ...
        for (srds_lane = 0; srds_lane < num_micros*2; srds_lane =(uint8_t)(srds_lane + 2)) {
            ...
            diag_data_err = osprey7_v2l8p1_uc_micro_var_dump(sa__, core, srds_lane, NULL);
            if(diag_data_err != ERR_CODE_NONE) {
                EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_UC_CORE returned error %s ****\n", osprey7_v2l8p1_INTERNAL_e2s_err_code(diag_data_err)));
                diag_level_error |= SRDS_DIAG_UC_CORE;
            }
    ...
    if(diag_level & SRDS_DIAG_UC_LANE) {
        ...
        diag_data_err = osprey7_v2l8p1_uc_lane_static_var_dump(sa__, core, lane, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
            EFUN_PRINTF(("\n**** WARNING: SRDS_DIAG_UC_LANE_STATIC returned error %s ****\n", osprey7_v2l8p1_INTERNAL_e2s_err_code(diag_data_err)));
            diag_level_error |= SRDS_DIAG_UC_LANE;
        }
        diag_data_err = osprey7_v2l8p1_uc_lane_var_dump(sa__, core, lane, NULL);
        if(diag_data_err != ERR_CODE_NONE) {
