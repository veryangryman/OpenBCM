...
err_code_t osprey7_v2l8p1_INTERNAL_read_lane_state(srds_access_t *sa__, osprey7_v2l8p1_lane_state_st *istate) {
  ...
  {
    uint8_t pfh1_ctrl;
    ESTM(state.pfm1_ctrl = rdv_pfm1_ctrl_val());
    ESTM(state.pfm2_ctrl = rdv_pfm2_ctrl_val());
    ESTM(pfh1_ctrl = rdv_pfh1_ctrl_val());
    ESTM(state.pfh_ctrl = (uint8_t)(pfh1_ctrl + rdv_pfh2_ctrl_val()));
  }
  ...
  EFUN(osprey7_v2l8p1_read_rx_afe(sa__, RX_AFE_FGA, &state.fga));
  ...
  EFUN(osprey7_v2l8p1_INTERNAL_get_tx_ppm(sa__, &state.tx_ppm));
...
err_code_t osprey7_v2l8p1_INTERNAL_display_lane_state_no_newline(srds_access_t *sa__) {
  ...
  if(state.osr_mode.tx_rx != 255) {
      const char *s = osprey7_v2l8p1_INTERNAL_get_osr_str(state.osr_mode.tx_rx, GET_OSR_STR_LONG);
      COMPILER_REFERENCE(s);
      EFUN_PRINTF(("(%2s%s, 0x%04x, 0x%02x ,", (state.br_pd_en) ? "BR" : "OS", s, state.ucv_config,state.ucv_status));
  } else {
      const char *s = osprey7_v2l8p1_INTERNAL_get_osr_str(state.osr_mode.tx, GET_OSR_STR_SHORT);
      const char *r = osprey7_v2l8p1_INTERNAL_get_osr_str(state.osr_mode.rx, GET_OSR_STR_SHORT);
      COMPILER_REFERENCE(s);
      COMPILER_REFERENCE(r);
      {
          char *t;
          uint8_t tx, rx;
  ...
  EFUN_PRINTF(("   (%2d,%2d,%1d,%2d) ", state.pfm1_ctrl, state.pfm2_ctrl, state.pf2_ctrl, state.pfh_ctrl));
