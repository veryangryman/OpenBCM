    ...
    int *local_lif)
{
    ...
    if (!(_SHR_IS_FLAG_SET(info->flags, BCM_SRV6_SRH_BASE_INITIATOR_REPLACE)))
    {
        ...
        global_lif_info.bta = dnx_data_lif.out_lif.global_srv6_outlif_bta_sop_get(unit) * info->nof_sids + 8;
        ...
        else if (reduce_mode_enabled == TRUE)
        {
            global_lif_info.bta = dnx_data_lif.out_lif.global_srv6_outlif_bta_sop_get(unit) * (info->nof_sids - 1) + 8;
        }
        else
        {
            global_lif_info.bta = dnx_data_lif.out_lif.global_srv6_outlif_bta_sop_get(unit) * info->nof_sids + 8;
