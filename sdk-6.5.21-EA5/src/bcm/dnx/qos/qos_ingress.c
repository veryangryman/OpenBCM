    ...
    switch (type)
    {
        ...
        case bcmQosControlMplsIngressSwapTtlModel:
        {
            ...
            if (dnx_data_headers.feature.feature_get(unit, dnx_data_headers_feature_fhei_mpls_cmd_from_eei))
            {
                ...
                for (mpls_command = DNX_EEI_IDENTIFIER_MULTI_SWAP_START;
                     mpls_command <= DNX_EEI_IDENTIFIER_MULTI_SWAP_LAST; mpls_command++)
                {
                    dbal_entry_value_field32_set(unit, entry_handle_id, DBAL_FIELD_FHEI_SWAP_TTL_MODEL, mpls_command,
                                                 ttl_mode);
                }
                SHR_IF_ERR_EXIT(dbal_entry_commit(unit, entry_handle_id, DBAL_COMMIT));
            }
    ...
    switch (type)
    {
        ...
        case bcmQosControlMplsIngressSwapTtlModel:
        {
            ...
            if (dnx_data_headers.feature.feature_get(unit, dnx_data_headers_feature_fhei_mpls_cmd_from_eei))
            {
                ...
                /**all mpls swap command use same ttl model*/
                SHR_IF_ERR_EXIT(DBAL_HANDLE_ALLOC
                                (unit, DBAL_TABLE_EGRESS_MPLS_FHEI_TO_EES_RESOLUTION, &entry_handle_id));
                mpls_command = DNX_EEI_IDENTIFIER_MULTI_SWAP_START;
                dbal_value_field32_request(unit, entry_handle_id, DBAL_FIELD_FHEI_SWAP_TTL_MODEL, mpls_command,
                                           &ttl_mode);
                SHR_IF_ERR_EXIT(dbal_entry_get(unit, entry_handle_id, DBAL_COMMIT));
            }
            else
