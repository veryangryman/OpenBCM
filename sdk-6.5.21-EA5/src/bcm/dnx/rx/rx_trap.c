    ...
    dnx_rx_trap_erpp_config_t * erpp_config_p)
{
    ...
    if (config_p->flags & BCM_RX_TRAP_UPDATE_DEST)
    {
        ...
        else
        {
            ...
            erpp_config_p->pp_dsp = pp_dsp;
            erpp_config_p->pp_dsp_core = core_id;
...
/**
 * \brief
 * This function receives ERPP forward action profile, core_id and clears indicated entry in HW
 * \param [in] unit -The unit number.
 ...
 */
static shr_error_e
dnx_rx_trap_erpp_action_hw_clear(
    int unit,
    int profile,
    uint32 core_id)
{
    ...
    dnx_rx_trap_erpp_config_t * erpp_config_p)
{
    ...
    for (core_id = 0; core_id < nof_cores; core_id++)
    {
        /** Clear entry before setting configuration */
        SHR_IF_ERR_EXIT(dnx_rx_trap_erpp_action_hw_clear(unit, profile, core_id));
        ...
        if (core_id == erpp_config_p->pp_dsp_core)
        {
...
* from HW the destination configured.
* \param [in] unit -The unit number.
* \param [in] profile - Holds the HW profile index.
    ...
    dnx_rx_trap_erpp_config_t * erpp_config_p)
{
    ...
    for (core_id = 0; core_id < nof_cores; core_id++)
    {
        ...
        if (pp_dsp_valid)
        {
            erpp_config_p->pp_dsp_valid = pp_dsp_valid;
            erpp_config_p->pp_dsp_core = core_id;
            erpp_config_p->pp_dsp = pp_dsp;
        }
    }
    ...
    dnx_rx_trap_erpp_config_t * erpp_config_p)
{
    ...
    SHR_IF_ERR_EXIT(dnx_rx_trap_erpp_dest_hw_get(unit, profile, erpp_config_p));
exit:
    DBAL_FUNC_FREE_VARS;
    SHR_FUNC_EXIT;
}
    ...
    bcm_rx_trap_config_t * config_p)
{
    ...
    if (erpp_config_p->pp_dsp_valid == TRUE)
    {
        SHR_IF_ERR_EXIT(dnx_algo_port_pp_dsp_to_logical_get
                        (unit, erpp_config_p->pp_dsp_core, erpp_config_p->pp_dsp, &logical_port));
        config_p->flags |= BCM_RX_TRAP_UPDATE_DEST;
    ...
    bcm_rx_trap_protocol_profiles_t * protocol_profiles_p)
{
    ...
    if (protocol_trap_mode == DNX_RX_TRAP_PROTOCOL_MODE_IN_LIF)
    {
        if (BCM_L3_ITF_TYPE_IS_RIF(port) && !BCM_GPORT_IS_SET(port))
        {
            port = BCM_L3_ITF_VAL_GET(port);
            intf_type = ETH_RIF;
        ...
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange(unit, &in_lif_profile_info, old_in_lif_profile,
                                                    &new_in_lif_profile, intf_type, inlif_dbal_table_id));
    ...
    bcm_rx_trap_protocol_profiles_t * protocol_profiles_p)
{
    ...
    else
    {
        ...
        SHR_IF_ERR_REPLACE_AND_EXIT(dnx_algo_gpm_gport_to_hw_resources
                                    (unit, intf, DNX_ALGO_GPM_GPORT_HW_RESOURCES_LOCAL_LIF_INGRESS,
                                     &gport_hw_resources), _SHR_E_NOT_FOUND, _SHR_E_PARAM);
        inlif_dbal_table_id = gport_hw_resources.inlif_dbal_table_id;
        intf_type = LIF;
    }

    SHR_IF_ERR_EXIT(dnx_in_lif_vsi_profile_get(unit, intf, (uint32 *) &old_in_lif_profile, &result_type, intf_type));
    SHR_IF_ERR_EXIT(in_lif_profile_info_t_init(unit, &in_lif_profile_info));
    sal_memcpy(&in_lif_profile_info.ingress_fields.protocol_trap_profiles, protocol_profiles_p,
               sizeof(bcm_rx_trap_protocol_profiles_t));
    SHR_IF_ERR_EXIT(dnx_in_lif_profile_exchange(unit, &in_lif_profile_info, old_in_lif_profile,
                                                &new_in_lif_profile, intf_type, inlif_dbal_table_id));
    SHR_IF_ERR_EXIT(dnx_in_lif_vsi_profile_set(unit, intf, new_in_lif_profile, result_type, intf_type));
