    ...
    void *field_data)
{
    ...
    if (old_in_lif_profile != new_in_lif_profile)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_in_lif_profile_dealloc(unit, old_in_lif_profile, &default_in_lif_profile, LIF));
    }
...
dnx_flow_field_encap_access_desc(
    int unit)
{
    return "Encap access is a mandatory field for initiator, cannot be replaced once set";
}
    ...
    void *field_data)
{
    ...
    if (flow_app_type == FLOW_APP_TYPE_TERM)
    {
        ...
        SHR_IF_ERR_EXIT(dnx_rx_trap_lif_ingress_profile_set(unit, &gport_hw_resources, termination_info->action_gport));
    }
    else if (flow_app_type == FLOW_APP_TYPE_INIT)
    {
        bcm_flow_initiator_info_t *initiator_info = (bcm_flow_initiator_info_t *) field_data;
    ...
    bcm_flow_terminator_info_t * terminator_info)
{
    ...
    if (!_SHR_IS_FLAG_SET(terminator_info->valid_elements_clear, BCM_FLOW_TERMINATOR_ELEMENT_ACTION_GPORT_VALID))
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_lif_ingress_profile_hw_set(unit,
                                                               gport_hw_resources.inlif_dbal_table_id,
                                                               gport_hw_resources.inlif_dbal_result_type,
                                                               gport_hw_resources.local_in_lif, new_lif_trap_profile));
    }
    ...
    bcm_flow_initiator_info_t * initiator_info)
{
    ...
    {
        SHR_IF_ERR_EXIT(dnx_rx_trap_lif_etpp_profile_lif_hw_set
                        (unit, &gport_hw_resources, new_lif_trap_profile, gport_hw_resources.local_out_lif));
    }
    ...
    void *field_data)
{
    ...
    if (flow_app_type == FLOW_APP_TYPE_TERM)
    {
        SHR_IF_ERR_EXIT(dnx_flow_field_action_gport_terminator_replace
                        (unit, gport_hw_resources, (bcm_flow_terminator_info_t *) field_data));
    }
    else if (flow_app_type == FLOW_APP_TYPE_INIT)
    {
        SHR_IF_ERR_EXIT(dnx_flow_field_action_gport_initiator_replace
                        (unit, gport_hw_resources, (bcm_flow_initiator_info_t *) field_data));
    }
