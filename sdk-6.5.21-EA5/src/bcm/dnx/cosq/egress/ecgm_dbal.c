    ...
    uint8 *erp_tc)
{
    ...
    dbal_value_field8_request(unit, entry_handle_id, DBAL_FIELD_ERP_FC_INDICATION, INST_SINGLE, erp_tc);
    ...
    uint8 *fc_indication)
{
    ...
    if (dnx_data_ecgm.info.feature_get(unit, dnx_data_ecgm_info_fc_egr_if_vector_select))
    {
        select =
            (interface +
             priority * dnx_data_egr_queuing.params.nof_egr_interfaces_get(unit)) /
