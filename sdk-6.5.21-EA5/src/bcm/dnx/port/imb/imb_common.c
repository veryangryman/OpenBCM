    ...
    int link)
{
    ...
    if (dnx_data_nif.features.feature_get(unit, dnx_data_nif_features_toggle_lag_failover_on_link_status_up) && link)
    {
        /*
         * Lag failover status toggle is required on link status up 
         */
        SHR_IF_ERR_EXIT(portmod_port_lag_failover_status_toggle(unit, port));
    }
