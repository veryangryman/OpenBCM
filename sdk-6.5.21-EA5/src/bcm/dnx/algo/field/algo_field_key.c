    ...
    dnx_field_key_id_t *key_id_p)
{
            ...
            if (flags & DNX_ALGO_FIELD_KEY_FLAG_WITH_ID)
            {
                ...
                if (key_id_iter > key_id_p->id[0])
                {
                    SHR_ERR_EXIT(_SHR_E_NOT_FOUND,
                                 ...
                                 "context_id %d\r\n",
                                 key_id_iter, (unsigned int) fg_type, (unsigned int) field_stage, context_id);
                }
            }
            /*
                ...
                if (key_length == DNX_FIELD_KEY_LENGTH_TYPE_HALF || key_length == DNX_FIELD_KEY_LENGTH_TYPE_DT)
                {
                    ...
                    if (key_id_p->id[0] > (dbal_enum_value_field_field_key_e) last_available_key)
                    {
                        SHR_ERR_EXIT(_SHR_E_NOT_FOUND,
                                     ...
                                     " context_id %d key_length %d. No key was assigned.\r\n",
                                     (unsigned int) first_available_key, (unsigned int) last_available_key,
                                     (unsigned int) fg_type, (unsigned int) field_stage, context_id, key_length);
                    }
                                    ...
                                    DNX_FIELD_KEY_OCCUPY_NONE)
                                {
                                    ...
                                    if (key_id_p->id[0] > (dbal_enum_value_field_field_key_e) (last_available_key - 1))
                                    {
                                        SHR_ERR_EXIT(_SHR_E_NOT_FOUND,
                                                     ...
                                                     (unsigned int) field_stage, context_id, key_length);
                                    }
                                    CONSTRUCT_KEY_ALLOCATION_ID(key_allocation_id, fg_type, field_stage, context_id,
    ...
    {
        SHR_ERR_EXIT(_SHR_E_NOT_FOUND,
                     "Could not find key for stage %d fg type %d context_id %d key_length %d.\r\n",
                     field_stage, fg_type, context_id, key_length);
    }

    {
        ...
        /** Parasitic allocation of new sw state. */
        {
            int key_length_bits_first;
            int key_length_bits_second;
            uint32 bit_offset = 0;
            switch (key_length)
            {
                case DNX_FIELD_KEY_LENGTH_TYPE_DT:
                    /*
                     * Would have made more sense to have dnx_data_field.map.key_size_get(unit)
                     * but this is how it currently goes.
                     */
                    key_length_bits_first = dnx_data_field.tcam.key_size_half_get(unit);
                    key_length_bits_second = 0;
                    break;
                ...
                case DNX_FIELD_KEY_LENGTH_TYPE_SINGLE:
                    key_length_bits_first = dnx_data_field.tcam.key_size_single_get(unit);
                    key_length_bits_second = 0;
                    break;
                case DNX_FIELD_KEY_LENGTH_TYPE_DOUBLE:
                    key_length_bits_first = dnx_data_field.tcam.key_size_single_get(unit);
                    key_length_bits_second =
                        dnx_data_field.tcam.key_size_double_get(unit) - dnx_data_field.tcam.key_size_single_get(unit);
                    break;
                    /*
                     * Coverity explanation: Switch should always have a default, for potential future changes.
                     */
                     /* coverity[dead_error_begin:FALSE]  */
                default:
                    SHR_ERR_EXIT(_SHR_E_INTERNAL, "Unknown key_legth %d\r\n", key_length);
            }
            SHR_IF_ERR_EXIT(dnx_algo_field_key_key_occupation_bmp_allocate
                            (unit, field_stage, context_id, key_length_bits_first, TRUE, FALSE, key_id_p->id[0],
                             &bit_offset));
            SHR_IF_ERR_EXIT(dnx_algo_field_key_key_lookup_occupation_bmp_allocate
                            (unit, field_stage, context_id, key_id_p->id[0]));
            if (key_length_bits_second != 0)
            {
                SHR_IF_ERR_EXIT(dnx_algo_field_key_key_occupation_bmp_allocate
                                (unit, field_stage, context_id, key_length_bits_second, TRUE, FALSE, key_id_p->id[1],
                                 &bit_offset));
                SHR_IF_ERR_EXIT(dnx_algo_field_key_key_lookup_occupation_bmp_allocate
    ...
    uint32 *bit_range_offset_within_key_p)
{
...

    {
        /** Parasitic allocation of new sw state. */
        SHR_IF_ERR_EXIT(dnx_algo_field_key_key_occupation_bmp_allocate
                        (unit, field_stage, context_id, key_length, TRUE, FALSE, key_id_p->id[0],
    ...
    dnx_field_key_id_t *key_id_p)
{
    ...
    /** Parasitic deallocation of new sw state. */
    {
        int key_size;
        if (key_id_p->full_key_allocated)
        {
            key_size = dnx_data_field.tcam.key_size_single_get(unit);
        }
        else
        {
            key_size = dnx_data_field.tcam.key_size_half_get(unit);
        }
        SHR_IF_ERR_EXIT(dnx_algo_field_key_key_occupation_bmp_deallocate
                        (unit, field_stage, context_id, key_size, key_id_p->id[0], 0));
        SHR_IF_ERR_EXIT(dnx_algo_field_key_key_lookup_occupation_bmp_deallocate
                        (unit, field_stage, context_id, key_id_p->id[0]));
        if (key_id_p->id[1] != DNX_FIELD_KEY_ID_INVALID)
        {
            SHR_IF_ERR_EXIT(dnx_algo_field_key_key_occupation_bmp_deallocate
                            (unit, field_stage, context_id, key_size, key_id_p->id[1], 0));
            SHR_IF_ERR_EXIT(dnx_algo_field_key_key_lookup_occupation_bmp_deallocate
                            (unit, field_stage, context_id, key_id_p->id[1]));
        }
    }
    ...
    uint32 bit_range_offset_within_key)
{
    ...
    /** Parasitic deallocation of new sw state. */
    SHR_IF_ERR_EXIT(dnx_algo_field_key_key_occupation_bmp_deallocate
                    (unit, field_stage, context_id, key_id.bit_range.bit_range_size, key_id.id[0],
 ...
 */
    ...
    }
    {
         ...
         */
        sal_memset(&dnx_field_key_alloc_detail, 0, sizeof(dnx_field_key_alloc_detail));
        /*
    ...
    uint32 *bit_lsb_p)
{
    ...
    else if (rv == _SHR_E_RESOURCE)
    {
        ...
        {
            LOG_DEBUG_EX(BSL_LOG_MODULE,
                         "Allocation not possible size 0x%X field_stage %d context %d key %d. Moving on to next key.\n",
                         size, field_stage, context_id, key_id);
        }
        else
        {
            LOG_DEBUG_EX(BSL_LOG_MODULE,
                         "Allocation failed size 0x%X field_stage %d context %d key %d. Moving on to next key.\n",
                         size, field_stage, context_id, key_id);
        }
    }
    else
    {
        SHR_ERR_EXIT(_SHR_E_INTERNAL,
                     "Failed to allocate key for size 0x%X field_stage %d context %d. Invalid error %d\n", size,
                     field_stage, context_id, rv);
    }
