    ...
    uint8 enable)
{
    ...
    if (BCM_GPORT_IS_TRUNK(gport))
    {
        ...
        for (core = 0; core < dnx_data_device.general.nof_cores_get(unit); ++core)
        {
            ...
            {
                SHR_IF_ERR_EXIT(dnx_switch_svtag_ptc_enable(unit, local_port, enable));
            }
        }
    }
    else
    {
        SHR_IF_ERR_EXIT(dnx_switch_svtag_ptc_enable(unit, BCM_GPORT_LOCAL_GET(gport), enable));
    }
