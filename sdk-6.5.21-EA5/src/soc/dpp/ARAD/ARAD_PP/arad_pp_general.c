  ...
  )
{
  ...
  case SOC_PPC_FRWRD_DECISION_TYPE_UC_PORT:
  case SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG:
      if ((fwd_decision->type == SOC_PPC_FRWRD_DECISION_TYPE_UC_LAG) && BCM_GPORT_SUB_TYPE_IS_TRUNK_MEMBER(fwd_decision->dest_id)) {
          sys_port_id = BCM_GPORT_SUB_TYPE_TRUNK_MEMBER_GET(fwd_decision->dest_id);
      } else {
  ...
  )
{
  ...
  {
    if (!BCM_GPORT_SUB_TYPE_IS_TRUNK_MEMBER(fwd_decision->dest_id)) {
