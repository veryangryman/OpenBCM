    ...
    *equal = FALSE;     
    ...
    if (key_size == DOUBLE_KEY_BIT_SIZE)
    {
        ...
        LOG_DEBUG_EX(BSL_LOG_MODULE, "Double-key comparision, comparing second half in odd bank ID%s%s%s%s\n", EMPTY,
                     EMPTY, EMPTY, EMPTY);
        SHR_IF_ERR_EXIT(dnx_field_tcam_access_entry_equal_shadow(unit, core, key + SINGLE_KEY_WORD_SIZE, key_mask + SINGLE_KEY_WORD_SIZE, key_size / 2, bank_id + 1, bank_offset, TRUE   , equal));
        if (!*equal)
        {
            SHR_EXIT();
        }
