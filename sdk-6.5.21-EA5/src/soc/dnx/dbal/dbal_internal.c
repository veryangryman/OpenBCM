    ...
    int unit)
{
    ...
    if (dnx_data_dbal.db_init.dbal_collection_pool_size_get(unit))
    {
        ...
        for (worker_thread_id = 0; worker_thread_id < DBAL_NOF_COLLECTION_POOLS; worker_thread_id++)
        {
            ...
            dbal_mngr[unit]->collect_info.worker_threads[worker_thread_id] = sal_thread_create(thread_name, 4 * SAL_THREAD_STKSZ, 50     ,
                                                                                               dbal_collection_worker_thread,
                                                                                               (void
                                                                                                *)
...
shr_error_e dbal_kbp_access_cache_commit(int unit, int is_acl){DBAL_KBP_DUMMY_PROTOTYPE(unit);}
