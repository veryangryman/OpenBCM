...
shr_error_e
_dbal_init_tables_hard_logic_pp_out_lif_profile_hl_pp_out_lif_profile_erpp_out_lif_bta_sop_table_init(
int unit,
table_db_struct_t * cur_table_param,
dbal_logical_table_t * table_info)
{
    dbal_logical_table_t * table_entry = table_info + DBAL_TABLE_ERPP_OUT_LIF_BTA_SOP_TABLE;
    int is_valid = dnx_data_dev_init.general.feature_get(unit, dnx_data_dev_init_general_egress_estimated_bta_btr_config);
    SHR_FUNC_INIT_VARS(unit);
    dbal_init_table_db_struct_clear(cur_table_param);
    SHR_IF_ERR_EXIT(dbal_db_init_general_info_set(unit, table_entry, DBAL_TABLE_ERPP_OUT_LIF_BTA_SOP_TABLE , is_valid , "ERPP_OUT_LIF_BTA_SOP_TABLE" , DBAL_ACCESS_METHOD_HARD_LOGIC ));
    if (!is_valid)
    {
        SHR_EXIT();
    }
    ...
    
    {
        ...
        dbal_table_field_info_t * dbal_db_field;
        table_db_field_params_struct_t db_field;
        SHR_ALLOC_SET_ZERO(table_entry->keys_info, 1 * sizeof(dbal_table_field_info_t), "key fields info allocation", "%s%s%s\r\n", "ERPP_OUT_LIF_BTA_SOP_TABLE" , EMPTY, EMPTY);
        
        {
            dbal_db_field = &table_entry->keys_info[field_index];
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_mandatory_values(unit, DBAL_FIELD_GLOBAL_OUT_LIF_MSB , DBAL_FIELD_TYPE_DEF_UINT , dbal_db_field));
            dbal_db_init_table_field_params_init(&db_field);
            db_field.size = utilex_log2_round_up(dnx_data_lif.global_lif.nof_global_out_lifs_get(unit))-utilex_log2_round_up(dnx_data_lif.out_lif.global_outlif_bta_sop_resolution_get(unit));
            SHR_IF_ERR_EXIT(dbal_db_init_table_field_set_optional_or_default_values(unit, table_entry, dbal_db_field, &db_field, TRUE));
            field_index += dbal_db_field->nof_instances;
        }
        table_entry->nof_key_fields = field_index;
    ...
    
    {
        ...
        uint8 is_compatible_with_all_images = TRUE;
        DBAL_DB_INIT_TABLE_IMAGE_SPECIFIC_TABLE_INDICATION_SET(table_entry, is_standard_1, is_compatible_with_all_images);
        SHR_IF_ERR_EXIT(dbal_db_init_table_set_table_incompatible_image_sw_state_indication(unit, DBAL_TABLE_ERPP_OUT_LIF_BTA_SOP_TABLE, is_standard_1, is_compatible_with_all_images));
    }
        ...
        
        {
            ...
            
            {
                ...
                
                {
                    ...
                    SHR_IF_ERR_EXIT(dbal_db_init_table_set_access_register_memory(unit, access_params, DBAL_HW_ENTITY_GROUP_EMPTY , regMemHwEntityId , fieldHwEntityId , INVALIDm ));
                    access_params->entry_offset.formula_cb = erpp_out_lif_bta_sop_table_bta_sop_entryoffset_0_cb;
                }
            }
        }
    ...
    SHR_IF_ERR_EXIT(dbal_db_init_table_add(unit, cur_table_param, DBAL_TABLE_ERPP_OUT_LIF_BTA_SOP_TABLE, table_info));
exit:
    SHR_FUNC_EXIT;
}
...
dbal_logical_table_t * table_info)
{
    ...
    SHR_IF_ERR_EXIT(_dbal_init_tables_hard_logic_pp_out_lif_profile_hl_pp_out_lif_profile_erpp_out_lif_bta_sop_table_init(unit, cur_table_param, table_info));
exit:
    SHR_FUNC_EXIT;
}
