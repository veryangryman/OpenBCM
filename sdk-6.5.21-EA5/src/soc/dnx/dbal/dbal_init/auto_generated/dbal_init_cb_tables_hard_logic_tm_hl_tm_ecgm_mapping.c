...
uint32 * offset)
{
    ...
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_EGQ_IF, key_value__egq_if);
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_ECGM_PRIORITY, key_value__ecgm_priority);
    *offset = (key_value__egq_if+key_value__ecgm_priority*dnx_data_egr_queuing.params.nof_egr_interfaces_get(unit))%dnx_data_ecgm.info.fc_egr_if_vector_nof_presented_get(unit);
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
