...
shr_error_e
erpp_out_lif_bta_sop_table_bta_sop_entryoffset_0_cb(
int unit,
void * entry_handle,
dbal_fields_e current_mapped_field_id,
uint32 * offset)
{
    dbal_entry_handle_t *  eh = (dbal_entry_handle_t *) entry_handle;
    uint32 key_value__global_out_lif_msb;
    SHR_FUNC_INIT_VARS(unit);
    DBAL_FORMULA_CB_GET_KEY_VALUE(unit, eh, DBAL_FIELD_GLOBAL_OUT_LIF_MSB, key_value__global_out_lif_msb);
    *offset = key_value__global_out_lif_msb;
    SHR_EXIT();
exit:
    SHR_FUNC_EXIT;
}
