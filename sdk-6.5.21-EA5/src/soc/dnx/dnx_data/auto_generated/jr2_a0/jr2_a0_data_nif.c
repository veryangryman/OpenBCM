...
static shr_error_e
jr2_a0_dnx_data_nif_eth_is_12G_supported_set(
    int unit)
{
    dnxc_data_define_t *define;
    int module_index = dnx_data_module_nif;
    ...
    int unit)
{
    ...
    if (17 < table->keys[0].size)
    {
        ...
        data->fec_type = bcmPortPhyFecNone;
        data->dispatch_type = portmodDispatchTypePm8x50;
        data->is_valid = dnx_data_nif.eth.is_12G_supported_get(unit) > 0 ? TRUE : FALSE;
    }
    if (18 < table->keys[0].size)
    {
    ...
    int unit)
{
    ...
    define = &submodule->defines[data_index];
    define->set = jr2_a0_dnx_data_nif_eth_is_12G_supported_set;
