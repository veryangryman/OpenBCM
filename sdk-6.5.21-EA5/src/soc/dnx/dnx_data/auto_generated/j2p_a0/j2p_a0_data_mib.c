    ...
    int unit)
{
    ...
    if (dnx_mib_counter_rx_fec_16_symbol_error_per_frame < table->keys[0].size && DNX_ALGO_PORT_MIB_COUNTER_SET_TYPE_FABRIC < table->keys[1].size)
    {
        data = (dnx_data_mib_controlled_counters_set_data_t *) dnxc_data_mgmt_table_data_get(unit, table, dnx_mib_counter_rx_fec_16_symbol_error_per_frame, DNX_ALGO_PORT_MIB_COUNTER_SET_TYPE_FABRIC);
        data->get_cb = dnx_mib_fmac_counter_get;
        data->clear_cb = dnx_mib_fmac_counter_clear;
    }
