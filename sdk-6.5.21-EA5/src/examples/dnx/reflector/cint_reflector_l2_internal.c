    ...
    uint32 *ace_entry_handle)
{
    ...
    ace_entry_info.entry_action[1].value[0] = bcmFieldAceContextReflectorL2Int;
        ...
        bcm_field_context_t epmf_context_id)
{
    ...
    bcm_field_group_info_t fg_info;
    bcm_port_interface_info_t interface_info;
    bcm_port_mapping_info_t mapping_info, mapping_info_1;
    uint32 flags;
    ...
    rv = bcm_port_get(unit, egress_port, &flags,&interface_info,&mapping_info_1);
    if (rv != BCM_E_NONE)
    {
    ...
    rv = cint_reflector_l2_internal_uc_fp_entry_add(unit, fg_id, &fg_info, in_port, mapping_info_1.tm_port, mapping_info.tm_port, ace_entry_handle);
    if (rv != BCM_E_NONE)
    {
        printf("cint_reflector_l2_internal_fp_entry_add failed\n");
        return rv;
