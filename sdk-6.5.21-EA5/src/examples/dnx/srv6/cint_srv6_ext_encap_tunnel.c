        ...
        int nof_sids_third_pass)
{
   ...
   if (rv != BCM_E_NONE)
   {
    ...
    add_host_ipv4(unit, cint_srv6_tunnel_info.route_ipv4_dip, cint_srv6_tunnel_info.vrf_in[INGRESS], 0, 0, gport);
    if (rv != BCM_E_NONE)
    {
        printf("%s(): Error, in function add_host_ipv4() for SRV6 Ingress Tunnel\n",proc_name);
        return rv;
    }
    add_host_ipv6_encap_dest(unit, cint_srv6_tunnel_info.route_ipv6_dip, cint_srv6_tunnel_info.vrf_in[INGRESS], 0, 0, gport);
    if (rv != BCM_E_NONE)
    {
    ...
    int nof_tatol_sids = nof_sids_first_pass + nof_sids_second_pass + nof_sids_third_pass;
    ...
    if(nof_sids_third_pass > 0)
    {
        ...
        /** set number of SIDs*/
        srh_base_info.nof_sids = nof_tatol_sids;
