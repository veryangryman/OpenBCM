...
int port_gpon_config(int unit, bcm_port_t port_gpon, uint8 num_gpon_channels)
{
	...
	printf("port_gpon_config: bcm_port_pon_tunnel_map_set port_gpon %d channel %d port_gpon_pp %d\n", port_gpon,channel, port_gpon_pp);
...
int port_gpon_1_1_service_create(int unit, 
                 bcm_port_t port_gpon_pp, 
                 uint32 tunnel,
                 uint32 qtag,
                 bcm_port_t port_nni,
                 uint32 stag)
{
    ...
    /* 
     * Create GPON AC-LIF
     * Service classification: {GPON-Port(5), GEM-ID(12), QTAG(12)}
     * GPON-Port is obtained from the Vlan Domain assigned to the GPON PP Port
     */
    gpon_vlan_port_id = 0;	/* Allow the SDK to allocate the LIF-Id */
    rv =  pon_lif_create(unit, port_gpon_pp, match_otag, 0, 0, tunnel, tunnel, qtag, 0, 0, 0, &gpon_vlan_port_id);
    ...
    cross_connect_gports.port1= gpon_vlan_port_id;
    cross_connect_gports.port2= nni_vlan_port_id;
