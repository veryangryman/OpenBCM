...
/**
 * All Endpoint cases must be resolved at ingress PMF, by building hte next DIP on UDH.
 ...
 *          presel is set based on FWD2 context, inhigher pririty than the cklassical end-point context
 *
 *   \param [in] unit - device id
 *   \param [in] ctx_id -  the context to which to attach the Presel
 *   \param [in] presel_id - the preselctor id
 *
 * \return
 *   \retval shr_error_e - negative in case of an error, zero in case all ok.
 */
    ...
    int unit)
{
    ...
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 16;
    sal_strncpy_s((char *) (qual_info.name), "srv6_usid_endpoint_udh_0_q", sizeof(qual_info.name));
    ...
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 80;
    sal_strncpy_s((char *) (qual_info.name), "srv6_usid_endpoint_udh_1_q", sizeof(qual_info.name));
    ...
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 32;
    sal_strncpy_s((char *) (qual_info.name), "srv6_usid_endpoint_udh_2_q", sizeof(qual_info.name));
    ...
    /*
     * take the last 128 bit from the packet, i.e metadata that's passed in the pipe
     * the 128 lsb is IPv6 DIP, that the Ingress writes to the metadata, offset 0
     */
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(field_srv6_context_set
                    (unit, bcmFieldStageIngressPMF1, "SRv6_srep_usid_udh_pmf1",
    ...
    int appl_psp_enable)
{
    ...
    SHR_IF_ERR_EXIT(field_srv6_pmf1_presel_set
                    (unit, cint_field_srv6_endpoint_info.ipmf1_context_id, 13, 0, (uint8) appl_psp_enable));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(field_srv6_pmf1_presel_set(unit, cint_field_srv6_egress_usp_info.ipmf1_context_id, 12, 1, 0));
    ...
    int unit)
{
    ...
    /** create the context and name it */
    SHR_IF_ERR_EXIT(field_srv6_context_set
                    (unit, bcmFieldStageIngressPMF1, "SRv6_srv6_extended_termination_btr", &context_id));

    /** create the Presel and attach it to the above context */
    bcm_field_presel_entry_id_info_init(&ipmf_presel_entry_id);
    bcm_field_presel_entry_data_info_init(&ipmf_presel_entry_data);

    ipmf_presel_entry_id.presel_id = 60;
    ipmf_presel_entry_id.stage = bcmFieldStageIngressPMF1;
    ipmf_presel_entry_data.entry_valid = TRUE;
    ipmf_presel_entry_data.context_id = context_id;
    ipmf_presel_entry_data.nof_qualifiers = 1;

     /** check if FWD layer is SRv6Beyond */
    ipmf_presel_entry_data.qual_data[0].qual_type = bcmFieldQualifyForwardingType;
    ...
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 8;
    sal_strncpy_s((char *) (qual_info.name), "srv6_extended_termination_btr", sizeof(qual_info.name));
    SHR_IF_ERR_EXIT(bcm_field_qualifier_create(unit, 0, &qual_info, &udh_qual));
    bcm_field_qualifier_info_create_t_init(&qual_info);
    qual_info.size = 2;
    sal_strncpy_s((char *) (qual_info.name), "srv6_extended_termination_btr_type", sizeof(qual_info.name));
    ...
    fg_info.stage = bcmFieldStageIngressPMF2;
    dest_char = &(fg_info.name[0]);
    sal_strncpy_s(dest_char, "Srv6_extended_termination_btr", sizeof(fg_info.name));
    ...
    action_info.prefix_value = 0;
    action_info.stage = bcmFieldStageIngressPMF2;
    sal_strncpy_s((char *) (action_info.name), "srv6 udh btr", sizeof(action_info.name));
    ...
    int unit)
{
    ...
    SHR_IF_ERR_EXIT(field_srv6_pmf1_presel_set(unit, cint_field_srv6_egress_psp_info.ipmf1_context_id, 14, 1, 1));
    ...
    int unit)
{
    ...
    {
        SHR_IF_ERR_EXIT(appl_ref_srv6_extended_termination_btr(unit));
    }
